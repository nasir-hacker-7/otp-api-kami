...
async function fetchOTP(phoneNumber) {
  try {
    ...
    console.log(`📦 API Response type: ${typeof data}, isArray: ${Array.isArray(data)}, length: ${Array.isArray(data) ? data.length : 'N/A'}`);

    if (!data) return { error: 'No data from API.' };

    // FORMAT 1: Array of Arrays  [[service, number, message, timestamp], ...]
    if (Array.isArray(data) && data.length > 0 && Array.isArray(data[0])) {
      console.log(`📋 Format detected: Array of Arrays (${data.length} records)`);
      for (const record of data) {
        ...
        if (numberMatches(recNum, cleanNumber)) {
          ...
          if (otp) {
            console.log(`✅ OTP [Array] for ${phoneNumber}: ${otp} | ${message}`);
            return { success: true, otp, service: record[0] || 'Unknown', message, timestamp: record[3] || null };
          }
        }
      }
      console.log(`❌ No OTP in ${data.length} array records for ${phoneNumber}`);
